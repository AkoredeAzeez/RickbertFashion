...
export async function createProduct(productData: CreateProductData, files: File[], onUploadProgress: (progress: any) => void): Promise<Product> {
  ...
  const productRes = await axios.post<{data: Product}>(`${API_URL}/api/products`, { data: product });
